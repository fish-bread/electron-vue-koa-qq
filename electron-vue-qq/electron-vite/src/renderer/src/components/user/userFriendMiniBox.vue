<script setup>
import { selectedUserUid, user_friend } from '../../function/user.js'
import { MiniUserFriendBox, showMiniUserFriendBox } from '../../function/boxclick/userFriendbox'
...
watchEffect(() => {
  if (showMiniUserFriendBox.value === true) {
    userFriend.value = user_friend.value.find(
      (user) => user.user_friend_uid === selectedUserUid.value
    )
...
<template>
  <div v-if="showMiniUserFriendBox" ref="MiniUserFriendBox" class="userMiniBox" @click.stop>
    <!--顶部用户信息-->
    <div class="user-top">
      <!--用户头像-->
      <div class="user-img">
        <img :src="userFriend.user_friend_headshot" alt="" />
      </div>
      <!--用户名字及uid-->
      <div class="user-name-box">
        <div class="user-name">{{ userFriend.user_friend_name }}</div>
        <div class="user-uid">uid:&nbsp;{{ userFriend.user_friend_uid }}</div>
      ...
      <div class="user-Thumbs-up-box">
        ...
        <div class="user-Thumbs-up-number">{{ userFriend.user_friend_thumbs_up }}</div>
      </div>
    </div>
    <!--中间各种东西-->
    <div class="user-center">
      ...
      <div class="user-center-level">
        ...
        <div class="user-center-text-right">LV:&nbsp; {{ userFriend.user_friend_level }}</div>
      </div>
      <!--签名-->
      <div class="user-center-level">
        <div class="user-center-text">签名</div>
        <div class="user-center-text-right">{{ userFriend.user_friend_personal_signature }}</div>
      </div>
      <!--所在地-->
      <div class="user-center-level">
        <div class="user-center-text">所在地</div>
        <div class="user-center-text-right">{{ userFriend.user_friend_region }}</div>
      </div>
    </div>
    <!--底部按钮-->
    <div class="user-bottom">
      <!--编辑资料-->
      <button class="user-bottom-button">音视频聊天</button>
    </div>
  </div>
</template>
