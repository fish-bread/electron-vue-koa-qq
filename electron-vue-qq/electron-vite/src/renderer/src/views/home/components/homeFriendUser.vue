...
import { searchUserAxios } from '../../../axios/search'
...
<template>
  <div class="home-friend-user">
    ...
    <div class="home-friend-user-friend-list">
      <div
        v-for="(user, index) in user_friend"
        :key="index"
        class="home-friend-user-friend-list-user"
        :class="{ selected: selectedUserUid === user.user_friend_uid }"
        @click="selectedUser(user.user_friend_uid)"
      >
        <div class="home-friend-user-friend-list-user-img">
          <img :src="user.user_friend_headshot" alt="" />
        </div>
        <div class="home-friend-user-friend-list-user-title">
          <div class="home-friend-user-friend-list-user-name">{{ user.user_friend_name }}</div>
          <div class="home-friend-user-friend-list-user-local">
            {{ user.user_friend_local_name }}
          </div>
        </div>
      </div>
    </div>
...
.selected {
  background-color: #0099ff !important; /* 蓝色背景 */
  color: white; /* 文字颜色 */
}
