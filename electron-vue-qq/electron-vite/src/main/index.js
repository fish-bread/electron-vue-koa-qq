...
import { createFullContextMenu, createSimpleContextMenu } from './menu'
import { trayFunc } from './tray'
import { clearImageCache, handleImageDownload, interceptImageRequests } from './webResquest'
...
// 主窗体
export let mainWindow = null // 添加主窗口引用
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('open-file-location', async (event, filePath) => {
    try {
      ...
      // 检查路径是否存在
      try {
        // 使用fs.promises.access检查路径可访问性
        await fs.promises.access(dirPath, fs.constants.F_OK | fs.constants.R_OK)
      } catch (accessError) {
  ...
  protocol.registerFileProtocol('app-image', (request, callback) => {
    const url = request.url.substr(12) // 移除 'app-image://'
  ...
  // 添加IPC处理网络图片请求
  ipcMain.handle('get-local-image-path', async (event, imageUrl) => {
    try {
