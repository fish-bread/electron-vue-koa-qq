import { selectedUserUid, user, user_friend_history_one, user_token } from '../function/user'
      ...
      if (res.data.user_history.user1.user1_uid === user.value.user_uid) {
        ...
        user_friend_history_one.value = {
          user: {
            ...
            user_headshot: `http://localhost:3000${res.data.user_history.user1.user1_headshot}?_t=${Date.now()}`
          },
          user_friend: {
            user_friend_uid: res.data.user_history.user2.user2_uid,
            user_friend_name: res.data.user_history.user2.user2_name,
            user_friend_headshot: `http://localhost:3000${res.data.user_history.user2.user2_headshot}?_t=${Date.now()}`
          },
        ...
        console.log('存储历史信息', user_friend_history_one.value)
        ...
        user_friend_history_one.value = {
          user: {
            ...
            user_headshot: `http://localhost:3000${res.data.user_history.user2.user2_headshot}?_t=${Date.now()}`
          },
          user_friend: {
            user_friend_uid: res.data.user_history.user1.user1_uid,
            user_friend_name: res.data.user_history.user1.user1_name,
            user_friend_headshot: `http://localhost:3000${res.data.user_history.user1.user1_headshot}?_t=${Date.now()}`
          },
          user_history: res.data.user_history.user_history.map((message) => ({
            user_take: message.sender_uid === user.value.user_uid ? message.message : null,
            friend_user_take: message.sender_uid === selectedUserUid.value ? message.message : null,
